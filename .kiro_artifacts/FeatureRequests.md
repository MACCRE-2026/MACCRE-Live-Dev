...
---
### Feature Name: A lane and a gather scope are different facts — the readout stops miscounting lanes in both directions
**Abstract:** `total_sum_readout`'s `lane_count` was `len(distinct Tether_ID)`, which had been wrong **twice for the same 8-agent scatter and in opposite directions**: **1** before per-lane tethers, which through `min(lane_count, cap)` promised the operator **one thread for an 8-way run**; then **9** after 4c-3, counting the group tether as a tenth lane. Now **8**. A lane is a tether with a parent; the group is the **gather scope** the lanes report into, carried by the scatter and its merge.
**Date/Time Entered:** 2026-09-06T12:55:00-04:00
**Status:** COMPLETED — task 4e. **The readout still has no caller; no operator sees these numbers yet.**
**Completed:** 2026-09-06T12:55:00-04:00
**Completion Metric:** `lane_tethers`, `gather_scopes`, the fan-out fallback and
`lane_count_source` in `total_sum_readout` (`maccre_core/orchestration/flow_engine.py`);
`nodes_per_lane` keyed by lane tethers only. `TestLanesAreNotGatherScopes` in
`tests/test_prelaunch_validation.py` — **10 tests**. The **4e red marker removed** after it
XPASSed. Gate 2026-09-06: `omni clean` 12:43 (303 bytecode), `omni qa` **PASS whole project**
12:44:47, pytest **1431 collected / 1429 passed / 2 xfailed / 0 failed** in 206.44 s,
`omni smoke` **ALL CHECKS PASSED** ($0.00). Reconciles against 1420/1417/3:
`1420 + 10 + 1 = 1431`, `1417 + 11 + 1 = 1429`, xfailed `3 - 1 = 2`. **Only two red markers
remain in the whole suite**: Req 34.1 and Req 31.6.

| | `lane_count` | `expected_peak_concurrency` |
|---|---|---|
| before per-lane tethers | **1** | **1** — one thread promised for an 8-way run |
| after 4c-3 | **9** | 8 (correct, as a side effect) |
| **after 4e** | **8** | **8** |

**A lane is a tether with a parent.** One line — `lane_group(t) != t` — and it is the whole
definition. Because it reads through `lane_group` it inherits the level-by-level comparison
rather than a prefix test, so `X.10` is not mistaken for something under `X.1`.

*** IT ALSO FIXED THE LEGACY CASE, WHICH I HAD NOT PLANNED FOR ***
A hand-authored CSV with one flat tether identifies no lane individually, so the tether
column cannot answer the question at all. The scatter's **fan-out width** can, and gives the
same answer. That topology went from `lane_count == 1` and a peak of **1** to **8 and 8**.

**Two sources of evidence for one question, with the source reported.**
`lane_count_source` names it: `"lane_tethers"` (direct), `"scatter_fan_out"` (inferred from
edges), or `"none"`. Taking a number from a different place without saying so is how an
unattributable count comes to be trusted. **This is not a second definition of a lane; it is
one question answered from the best available evidence, labelled** — and the precedent is in
the same function, where `source` already reports whether the rows are hydrated rather than
asserting it. Lane tethers win when both are present, being the direct evidence.

**`nodes_per_lane` excludes the control nodes, and the gap is asserted.** The scatter and
merge sit on the scope, not in a lane, so summing it gives 8 against a `node_count` of 10.
That difference is informative rather than a miscount, so a test pins
`sum == node_count - 2` and it becomes a documented property.

**Four pre-existing readout tests were left untouched and still pass** — the tether-count
test, the 64-lane clamp, the pool-request test and the hard-cap test — because they describe
lanes with hierarchical tethers (`X.0`..`X.63`) and the new definition agrees with them.
**That is the check that this is a correction rather than a redefinition: the tests that were
right did not move.**

**REVERT-TO-RED:** restoring `len(tethers)` failed **5 tests** — the 8-lane count, both
flat-topology tests, the two-scopes test and the former marker — while the peak test
**stayed green**, correctly, since `min(9, 8)` is still 8 and the peak was already fixed by
4c-3.

*** omni qa EARNED ITS KEEP AGAIN, AND THE FAILURE MODE IS WORTH RECORDING ***
Ruff caught three `F821 Undefined name 'Any'` in `test_prelaunch_validation.py`: I wrote
`list[dict[str, Any]]` without importing `Any`, and because that file has
`from __future__ import annotations` the annotations are never evaluated — **all 36 tests in
it passed while the name was undefined.** A green suite could not have found it. This is the
**second time in the 4c–4e sequence** that the whole-project gate caught something the tests
structurally could not; the first was `str | None` passed into `scope_tether: str` in 4c-1.

**Rejected:** deriving `lane_count` from the fan-out in all cases (it needs a `CTRL_SCATTER`
row, and four existing tests legitimately describe lanes with tether-only rows); keeping
`lane_tether_ids` as every distinct tether plus a separate lane list (two lists, one a
superset); reporting 0 lanes for a flat topology and stopping (honest but needlessly weak);
counting the group as a lane when it is the only tether — **that is the original defect
restated as a special case.**

**Contract change** on the returned dict: two new keys (`lane_count_source`,
`gather_scopes`), two changed meanings (`lane_tether_ids` is lanes only; `nodes_per_lane` is
keyed by lane tether only).

**LIMITS. The readout still has no caller** — `total_sum_readout` is not invoked by any
launch path, so no operator sees these numbers yet. Fixing the arithmetic before it has a
consumer is deliberate: it was already wrong, and a wrong number is worse once someone is
reading it. **`gather_strategies`, `waits` and `cross_lane_routes` are still empty** — Reqs
29/31/32 built the mechanisms, nothing populates these keys. **No live run has produced a
readout**; every number here comes from hand-built rows or the auto-wrap called directly, and
the 8-lane live run remains the outstanding evidence for the whole 4c–4e sequence. **The
legacy `DET_SCATTER` alias is not matched** by the fan-out fallback — `_resolve_node_type`
handles that normalisation but is private to `deterministic_nodes`, and reaching into another
module's private helper for an alias the auto-wrap never emits was the worse trade.
