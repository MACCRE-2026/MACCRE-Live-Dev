...
class MacroNodeBuilderPanel(Vertical):
    """Panel to create or edit a Template MacroNode."""
    
    class MacroSaved(Message):
    ...
    def refresh_data(self) -> None:
        ...
        from maccre_tui.nexus_plex import load_agent_names_from_library

        store = SQLiteMacroNodeStore(_db_path(self.active_project))
        templates_idx = store.list_all()
        self.templates = []
        for t in templates_idx:
            try:
                self.templates.append(store.load(t["name"]))
            except Exception:
                pass

        self.roster = sorted(load_agent_names_from_library(self.active_project))
        self.all_agents = sorted(self.roster + [sn[0] for sn in self.special_nodes])

        try:
            sel = self.query_one("#me-macro-select", Select)
            val = sel.value
            options = [("*** Create New... ***", "__NEW__")] + [(t["name"], t["name"]) for t in self.templates]
            sel.set_options(options)
            if val in [o[1] for o in options]:
                sel.value = val
            else:
                sel.value = "__NEW__"
        except Exception:
    ...
    def compose(self) -> ComposeResult:
        with Horizontal(id="me-title-row"):
        ...
        with Horizontal(classes="me-slot-row"):
            yield Label("Macro Name:", classes="me-slot-label")
            yield Input(id="me-name", classes="me-slot-input")

        with Horizontal(classes="me-slot-row"):
            yield Label("Description:", classes="me-slot-label")
            yield Input(id="me-desc", classes="me-slot-input")

        with Horizontal(classes="me-slot-row"):
            yield Label("Template Type:", classes="me-slot-label")
            type_opts = [(k, k) for k in TEMPLATE_CATALOG.keys()]
            yield Select(type_opts, prompt="Select Template...", id="me-type-select", classes="me-slot-input")

        yield Vertical(id="me-dynamic-container")

        with Horizontal(id="me-buttons"):
            yield Button("Cancel/Clear", variant="error", id="cancel-btn")
    ...
    async def on_type_select(self, event: Select.Changed) -> None:
        ...
        widgets_to_mount = []
        widgets_to_mount.append(Label(f"Agent Slots for {val}", classes="pane-title"))
        for slot in tpl_def.slots:
            ...
            widgets_to_mount.append(Horizontal(lbl, inp, classes="me-slot-row"))
            
            from textual.containers import Vertical
            tools_container = Vertical(id=f"tools_container_{slot.name}", classes="me-tools-container")
            widgets_to_mount.append(tools_container)
            self.set_timer(0.1, lambda s=slot.name: self.update_slot_tools_ui(s))
            
        await container.mount(*widgets_to_mount)
        widgets_to_mount.clear()
        
        if tpl_def.config:
            widgets_to_mount.append(Label(f"Configuration for {val}", classes="pane-title"))
            for cfg in tpl_def.config:
                ...
                if cfg.name == "structural_augment":
                    ...
                    h_container = Vertical(inp, classes="augment-container")
                    
                    widgets_to_mount.append(lbl)
                    widgets_to_mount.append(h_container)
        ...
        # Force the panel to recalculate its scroll region after dynamic mounts
        self.refresh(layout=True)
        self.set_timer(0.2, self._recalc_virtual_size)
    ...
    def _recalc_virtual_size(self) -> None:
        """Recalculate virtual_size so the scrollbar reflects dynamically mounted content."""
        from textual.geometry import Size
        total_height = 0
        for child in self.children:
            try:
                total_height += child.outer_size.height
            except Exception:
                pass
        width = self.size.width if self.size.width else 80
        new_virtual = Size(width, max(total_height, self.size.height))
        if new_virtual != self.virtual_size:
            self.virtual_size = new_virtual
    ...
    def action_cancel(self) -> None:
        self.query_one("#me-macro-select", Select).value = "__NEW__"
    ...
    def save(self):
        ...
        self.post_message(self.MacroSaved(result))
    ...
    def update_agent_details_panel(self) -> None:
        ...
        try:
            ...
            return  # Info panel not mounted (will be in overlay)
